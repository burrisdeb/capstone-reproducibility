...
str(dat)
```
As we can see dat is a data frame with two variables: agreement and condition  .
...
between agreement and condition, I performed a chi-square test.

```{r}
chisq.test(dat$agreement,dat$condition)
```
According to the results of the chi-square test, agreement and condition
are independent of one another Chi-square(1) = 1.2, p = .26.
